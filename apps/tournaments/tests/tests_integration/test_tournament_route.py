...
class TestTournamentRoute(APITestCase):
    ...
    def setUpClass(cls):
        ...
        cls.tournament_post_form = {
            "tournament_name": "test_tournament",
            "players_list": []
        }

        cls.tournament_put_form_wo_lock = {
            "tournament_name": "modified_test_tournament",
            "players_list": [1, 2, 3, 4, 5, 6, 7, 8]
        }

        cls.tournament_put_form_wo_lock_wrong_player = {
            "tournament_name": "modified_test_tournament",
            "players_list": [11]
        }

        cls.tournament_put_form_wo_lock_same_player = {
            "tournament_name": "modified_test_tournament",
            "players_list": [1, 1]
        }

        cls.tournament_put_form_with_lock_good_list = {
            "tournament_name": "modified_test_tournament",
            "players_list": [1, 2, 3, 4, 5, 6, 7, 8],
            "locked": True
        }

        cls.tournament_put_form_with_lock_bad_list = {
            "tournament_name": "modified_test_tournament",
...
class TestOpenTournamentRoute(TestTournamentRoute):
    ...
    def test_tournament_retrieve(self):
        response = self.client.get(reverse('tournaments-detail', kwargs=self.open_kwargs))
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_tournament_post(self):
        response = self.client.post(reverse('tournaments-list'),
                                    data=self.tournament_post_form)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_tournament_put_without_lock(self):
        response = self.client.put(reverse('tournaments-detail', kwargs=self.open_kwargs),
                                   data=self.tournament_put_form_wo_lock)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_tournament_put_without_lock_player_not_found(self):
        response = self.client.put(reverse('tournaments-detail', kwargs=self.open_kwargs),
                                   data=self.tournament_put_form_wo_lock_wrong_player)
        self.assertIn(b'The following player IDs do not exist', response.content)

    def test_tournament_put_without_lock_same_player(self):
        response = self.client.put(reverse('tournaments-detail', kwargs=self.open_kwargs),
                                   data=self.tournament_put_form_wo_lock_same_player)
        self.assertIn(b'The same ID is present several times', response.content)

    def test_tournament_put_with_lock_good_list(self):
        response = self.client.put(reverse('tournaments-detail', kwargs=self.open_kwargs),
                                   data=self.tournament_put_form_with_lock_good_list)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_tournament_put_with_lock_incomplete_list(self):
        response = self.client.put(reverse('tournaments-detail', kwargs=self.open_kwargs),
                                   data=self.tournament_put_form_with_lock_bad_list)
        self.assertIn(b'The players list is incomplete', response.content)

    def test_tournament_delete(self):
        response = self.client.delete(reverse('tournaments-detail', kwargs=self.open_kwargs))
...
class TestLockedTournamentRoute(TestTournamentRoute):
    ...
    def test_locked_tournament_edit(self):
        response = self.client.put(reverse('tournaments-detail', kwargs=self.locked_kwargs),
                                   data=self.tournament_post_form)
        self.assertIn(b'A locked tournament cannot be modified', response.content)
