...
class TestRoundRoute(APITestCase):
    ...
    def test_round_list(self):
        response = self.client.get(reverse('rounds-list', kwargs=self.list_kwargs))
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_round_retrieve(self):
        response = self.client.get(reverse('rounds-detail', kwargs=self.detail_kwargs))
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_round_post(self):
        response = self.client.post(reverse('rounds-list', kwargs=self.list_kwargs),
                                    data={})
        self.assertEqual(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

    def test_round_put(self):
        response = self.client.put(reverse('rounds-detail', kwargs=self.detail_kwargs),
                                   data={})
        self.assertEqual(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

    def test_round_delete(self):
        response = self.client.delete(reverse('rounds-detail', kwargs=self.detail_kwargs))
        self.assertEqual(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)
